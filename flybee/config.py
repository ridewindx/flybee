...
class SettingMeta(type):
    def __new__(mcs, name, bases, cls_dict):
        ...
        new_class.help = new_class.__doc__.splitlines()[0]
...
class Setting(object):
    ...
    action = None  # 'store', 'store_const', 'store_true', 'store_false', 'append', 'append_const', 'count'
    type = None
    nargs = None  # N (an integer), '?', '*', '+', argparse.REMAINDER
    const = None
    choices = None # an iterable
    metavar = None
    default = None  # uesd when optional argument is omitted; argparse.SUPPRESS causes no attribute to be added
    help = None
    desc = None
...
class Bind(Setting):
    ...
    default = '%s:%s' % (os.environ.get('HOST') or '0.0.0.0', os.environ.get('PORT') or 8000)
